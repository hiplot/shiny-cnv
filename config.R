...
# change if necessary
dataDir <- sprintf("%s/snpdata", tempdir())
demo_files <- list.files("SNPdata")
dir.create(dataDir)
file.symlink(file.path(getwd(), "SNPdata", demo_files),
  file.path(dataDir, demo_files))
...
getSmoothLine <- function(snpData, smoothNum = 10) {
  ...
  snpData %>%
    ...
    group_by(smtGroup) %>%
    summarise(medPos = median(Pos), medLRR = median(LRR)) %>%
    ungroup()
}
...
getKaryoData <- function(file) {
  karyoColors <- c(
    acen = "pink4", gneg = "grey20", gpos100 = "grey35", gpos75 = "grey45",
  ...
  chrLen <- karyoData %>%
    ...
    ungroup()
  chrPosOffset <- chrLen %>%
    mutate(posOffset = cumsum(as.double(end)), posOffset = lag(posOffset, default = 0)) %>%
    .$posOffset %>%
    as.vector() %>%
    set_names(unique(chrLen$chr))
  chrLabelDF <- chrLen %>%
    mutate(mid = end / 2 + chrPosOffset[chr], end2 = end + chrPosOffset[chr]) %>%
    as.data.frame()
...
getLRRcolor <- function(snpData) {
  ...
  snpData %>%
    mutate(
      normLRR = round(normLRR, digits = 3),
