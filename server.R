...
function(input, output, session) {
  ...
  observeEvent(input$SNPdata, {
    fn <- as.character(input$SNPdata$datapath)
    file.symlink(fn, file.path(dataDir, input$SNPdata$name))
  })
  ...
  observeEvent(input$getHgVersion, {
    if (!rv$SNPdataStatus) {
      ...
      rv$geneRefData <- read_tsv(geneRefFile, col_types = "cciiccc", progress = F) %>%
        group_by(chr) %>%
  ...
  observeEvent(input$cnvFile, {
    # fast read
    rawInput <- fread(input$cnvFile$datapath, header = T)
    if ("caseID" %in% colnames(rawInput)) {
      rv$inputFileText <- NULL
      rv$cnvDF <- rawInput %>%
        mutate(chr = ifelse(chr == 23, "X", ifelse(chr == 24, "Y", chr)), note = "False", updated = "")
      rv$sampleInfor <- rv$cnvDF %>%
        select(caseID, controlID, gender) %>%
        distinct(caseID, controlID, gender)
  ...
  observeEvent(input$getSNPdata, {
    if (!is.null(rv$cnvDF) & !rv$SNPdataStatus) {
      ...
      withProgress({
        ...
        for (i in 1:smpNum) {
          si <- samples[i]
          siFile <- paste0(dataDir, "/", si, input$dataSuffix)
          if (!file.exists(siFile)) {
            ...
            rv$SNPdata[[si]] <- data.table(read_tsv(siFile, col_types = "ccddd", na = c("", "NA", "na", "NaN"), progress = F)) %>%
              set_colnames(cnvDataHeader) %>%
              .[!is.na(LRR) & !is.na(BAF)] %>%
              .[, c("LRR", "normLRR", "BAF", "newPos", "Chr") := list(
                if_else(LRR < minLRR, minLRR, if_else(LRR > maxLRR, maxLRR, LRR)),
  ...
  observeEvent(input$getChrPos, {
    if (!is.null(rv$cnvItem) & input$getChrPos != "NA") {
      ...
      rv$clickedPos <- rv$chrRefData %>%
        filter(chr == cnvChr) %>%
        select_(.dots = chrTail) %>%
        as.integer()
    }
  })
  ...
  observe({
    if (!is.null(rv$zoomRate) & !is.null(rv$cnvItem)) {
      chrI <- rv$cnvItem$chr
      chrMax <- rv$karyoList$chrLabelDF %>%
        filter(chr == chrI) %>%
        .$end
      isolate({
  ...
  observeEvent(input$showCNV, {
    rv$cnvSegs <- cnvDFout() %>%
      select(caseID:CN) %>%
      mutate(
        newStart = start + rv$chrPosOffset[chr], newEnd = end + rv$chrPosOffset[chr], ymin = rv$caseYmin[caseID], ymax = ymin + CNV_hight, length = end - start + 1,
  ...
  cnvTblProxy <- DT::dataTableProxy("cnvTbl")
  observe({
    DT::replaceData(cnvTblProxy, cnvDFout(),
      rownames = F,
      resetPaging = F,
      clearSelection = "none"
    )
  ...
  output$genePlot <- renderPlot({
    if (!is.null(rv$cnvItem) & rv$SNPdataStatus) {
      if (nrow(caseSNPDF()) >= 2) {
        ...
        segments(
          x0 = cnvRegionGeneDF()$start, x1 = cnvRegionGeneDF()$end,
      ...
      output$spectrumPlot <- renderPlot(height = caseNum * 15 + 250, expr = {
        if (rv$SNPdataStatus & !is.null(caseSpectDF())) { # duplicate but necessary
          if (rv$spectType == "genome") {
            ...
            karyoDF <- karyoList$karyoDF %>%
              filter(chr == spectChr) %>%
              mutate(lengthRatio = (end - start) / spectXlen)
            ...
            if (snpPerCase < maxRect) {
              ...
              tmpDF <- caseSpectDF() %>%
                ...
                arrange(Pos) %>%
                mutate(
                  x1 = Pos - padWidth, x2 = Pos + padWidth, x1 = if_else(x1 < lag(x2, default = 0), (lag(x2) + x1) / 2, x1),
            ...
            if (nrow(spectRegionGeneDF()) < 500 & nrow(spectRegionGeneDF()) >= 1) {
              segments(
                x0 = spectRegionGeneDF()$start, x1 = spectRegionGeneDF()$end, xdp = NULL,
